...
The app opens directly to the plan form. Enter a hangout name and use the prefilled dates and hours, or adjust them. The time zone is always visible. **Update your availability** shows recent hangouts on this device; a friend's invite opens straight to its grid. The sample **Game night, anyone?** link lets you try responses immediately. Add your name, click individual one-hour blocks or hold and drag a rectangle, and save. Start a drag on a selected block to clear that rectangle. The grid scrolls near its edges, while date and time labels stay pinned. Choose **Show times in** or **Use my time zone** to convert the grid and best-time results; unsaved selections keep their original slot identities. The best times rank by the number of available people, with earlier slots winning ties.
...
- Plans retain the organizer's time zone. Each viewer can convert the grid and results to any supported time zone, including date changes and fractional offsets. Responses always save the original slot indexes. New plans reject skipped/repeated organizer clock-change hours rather than guessing. Older plans containing those ambiguous labels remain available in the organizer's zone, with an explanatory message. Repeated hours in a viewer's zone are distinguished by UTC offset.
- Mouse, pen, and touch dragging select or clear rectangular blocks; ordinary clicks and keyboard activation also work. On touch devices, drag inside cells to paint and swipe outside cells or use scrollbars to scroll. Physical touch-device verification remains outstanding.
